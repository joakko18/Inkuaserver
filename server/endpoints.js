...
router.get('/proyectos', async (req, res) => {
  try {
    ...
    const query = `
      SELECT title, description, url FROM url;
    `;
...
router.post('/upload', upload.single('image'), async (req, res) => {
  try {
    ...
    const query = 'INSERT INTO url1(url) VALUES ($1) RETURNING *';
    const values = [url];
...
router.post('/saveProyecto', async (req, res) => {
  try {
    ...
    const query = 'INSERT INTO url (title, description, url) VALUES ($1, $2, $3) RETURNING *';
